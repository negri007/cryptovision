...
export const processAlerts = onSchedule({
  ...
  const getAssetPrice = async (type: 'crypto' | 'stock', symbol: string): Promise<number | null> => {
    ...
    try {
      if (type === 'crypto') {
        const snap = await db.collection('market/crypto/prices').doc(symbol.toUpperCase()).get();
        if (snap.exists) {
          const val = snap.data()?.price || null;
          if (val) priceCache[cacheKey] = val;
          return val;
        }
      } else {
        const snap = await db.collection('market/stocks/prices').doc(symbol.toUpperCase()).get();
        if (snap.exists) {
          const val = snap.data()?.price || null;
          if (val) priceCache[cacheKey] = val;
          return val;
  ...
  try {
    ...
    const alertsSnapshot = await db.collectionGroup('alerts').where('status', '==', 'active').get();
      ...
      if (isTriggered) {
        ...
        for (const channel of channels) {
          if (channel === 'email' && userEmail) {
            const emailSuccess = await sendGrid.sendEmail(
              userEmail,
              `Alerta de Preço: ${symbol.toUpperCase()}`,
              'd-alert-template-id', // dynamic template id placeholder
              {
          ...
          if (channel === 'push' && fcmTokens.length > 0) {
            try {
              ...
              const fcmRes = await getMessaging().sendEachForMulticast(message);
